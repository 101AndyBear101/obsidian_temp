...
DEFAULT_INI="""[ignore]\npatterns = agents/validators/**, agents/skills/**/references/**\n[rules]\npattern = ^[a-z0-9][a-z0-9._-]*$\nreserved = AGENTS.md, CONVENTIONS.md, README.md, INDEX.md, SKILL.md, LICENSE, .gitignore\ndaily_pattern = ^\\d{4}-\\d{2}-\\d{2}\\.md$\n"""
def validate(root:Path,config):
    patterns=parse_items(config.get("ignore","patterns",fallback="")); regex=re.compile(config.get("rules","pattern")); daily=re.compile(config.get("rules","daily_pattern")); reserved=set(parse_items(config.get("rules","reserved")))
    base=root.parent if root.is_file() else root; paths=[root] if root.is_file() else list(root.rglob("*")); issues=[]; checked=0
    for path in paths:
        rel=relative(path,root)
        if ignored(rel,patterns): continue
        checked+=1
        if path.is_file() and rel.startswith("journals/daily/"):
            if not daily.fullmatch(path.name): issues.append(Issue("error","NAME001","Daily-note filename must use YYYY-MM-DD.md.",rel))
            else:
                try: date.fromisoformat(path.stem)
                except ValueError: issues.append(Issue("error","NAME002","Daily-note filename is not a real calendar date.",rel))
        elif path.name not in reserved and not regex.fullmatch(path.name): issues.append(Issue("error","NAME003","Name must use lowercase compact characters.",rel))
