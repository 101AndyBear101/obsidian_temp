...
VALIDATOR_NAME="indexes"; DEFAULT_INI="""[ignore]\npatterns =\n"""; LINK=re.compile(r"(?<!!)\[\[[^\]]+\]\]")
def validate(root:Path,config):
    files=[p for p in iter_files(root,(".md",),parse_items(config.get("ignore","patterns",fallback=""))) if p.name=="INDEX.md"]; issues=[]
    for path in files:
        if not LINK.search(path.read_text(encoding="utf-8")): issues.append(Issue("warning","IDX001","Index has no wikilinks.",relative(path,root)))
