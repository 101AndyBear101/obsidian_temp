...
VALIDATOR_NAME = "relationships"
DEFAULT_INI = """[ignore]\npatterns =\n[rules]\npattern = ^[a-z0-9]+(?:[-_][a-z0-9]+)*$\nwarning_length = 20\n"""

def validate(root: Path, config):
    files = iter_files(root, (".md",), parse_items(config.get("ignore", "patterns", fallback="")))
    regex = re.compile(config.get("rules", "pattern")); limit = config.getint("rules", "warning_length")
    issues = []
    expected = {"project": ("area", str), "resource": ("project", list), "note": ("resource", list)}
    for path in files:
        rel = relative(path, root)
        try: fm, _ = load_frontmatter(path)
        except Exception: continue
        if not fm: continue
        pair = expected.get(fm.get("kind"))
        if not pair: continue
        field, wanted = pair; value = fm.get(field)
        if not value:
            issues.append(Issue("warning", "REL001", f"Consider adding {field!r} metadata to improve discovery.", rel, 1)); continue
        if not isinstance(value, wanted):
            issues.append(Issue("error", "REL002", f"{field!r} has the wrong data type.", rel, 1)); continue
        values = [value] if isinstance(value, str) else value
        for item in values:
            if not isinstance(item, str) or not regex.fullmatch(item):
                issues.append(Issue("error", "REL003", f"Invalid compact identifier in {field!r}: {item!r}.", rel, 1))
            elif len(item) > limit:
                issues.append(Issue("warning", "REL004", f"Identifier {item!r} exceeds {limit} characters.", rel, 1))
