...
VALIDATOR_NAME="duplicates"; DEFAULT_INI="""[ignore]\npatterns =\n[rules]\nsimilarity_threshold = 0.88\nignored_names = AGENTS.md, README.md, INDEX.md, SKILL.md\n"""
def validate(root:Path,config):
    files=iter_files(root,(".md",),parse_items(config.get("ignore","patterns",fallback=""))); threshold=config.getfloat("rules","similarity_threshold"); ignored_names=set(parse_items(config.get("rules","ignored_names",fallback=""))); files=[path for path in files if path.name not in ignored_names]; issues=[]
    for index,left in enumerate(files):
        for right in files[index+1:]:
            ratio=difflib.SequenceMatcher(None,left.stem.lower(),right.stem.lower()).ratio()
            if ratio>=threshold: issues.append(Issue("warning","DUP001",f"Filename is {ratio:.0%} similar to {relative(right,root)!r}.",relative(left,root)))
