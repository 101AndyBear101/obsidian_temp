...
VALIDATOR_NAME="templates"; DEFAULT_INI="""[ignore]\npatterns =\n[rules]\ntemplates_directory = templates\n"""
FENCE=re.compile(r"^ {0,3}(`{3,}|~{3,})([^\s`~]*)[ \t]*$",re.M)
def validate(root:Path,config):
    base=root.parent if root.is_file() else root; folder_name=config.get("rules","templates_directory")
    if root.is_file():
        files=[root] if "templates" in root.parts and root.suffix.lower()==".md" else []
    else:
        candidates=iter_files(root,(".md",),parse_items(config.get("ignore","patterns",fallback="")))
        prefix=folder_name.rstrip("/")+"/"
        files=[path for path in candidates if relative(path,root).startswith(prefix)]
    issues=[]
    for path in files:
        stack=[]; rel=relative(path,root)
        for number,line in enumerate(path.read_text(encoding="utf-8").splitlines(),1):
            match=FENCE.fullmatch(line)
            if not match: continue
            marker=match.group(1)
            if stack and not match.group(2): stack.pop()
            elif match.group(2): stack.append((marker,number))
        for _,line in stack: issues.append(Issue("error","TPL001","Unclosed Markdown code fence.",rel,line))
