...
VALIDATOR_NAME="skills"; DEFAULT_INI="""[ignore]\npatterns =\n[rules]\nskills_directory = agents/skills\nindex_file = INDEX.md\n"""
def validate(root:Path,config):
    patterns=parse_items(config.get("ignore","patterns",fallback="")); issues=[]
    if root.is_file():
        if root.name!="SKILL.md": return [],0
        skill_dirs=[root.parent]
        skills_root=next((parent for parent in root.parents if parent.name=="skills"),root.parent)
        index=skills_root/config.get("rules","index_file")
    else:
        folder=root/config.get("rules","skills_directory")
        if not folder.exists(): return [],0
        skill_dirs=[path for path in folder.iterdir() if path.is_dir() and not ignored(relative(path,root),patterns)]
        index=folder/config.get("rules","index_file")
    content=index.read_text(encoding="utf-8") if index.exists() else ""
    for item in skill_dirs:
        if not (item/"SKILL.md").exists(): issues.append(Issue("error","SKILL002","Skill folder has no SKILL.md.",relative(item,root)))
        if not re.search(rf"(?<![a-z0-9-]){re.escape(item.name)}(?![a-z0-9-])",content): issues.append(Issue("warning","SKILL003","Skill is not registered in the skills index.",relative(item,root)))
