...
VALIDATOR_NAME="attachments"; DEFAULT_INI="""[ignore]\npatterns =\n[rules]\nattachments_directory = files\nignore_targets = note, embed, Note, Note Name, proxmox-nfs-settings, MyBase.base, BaseFile.base, image.png, document.pdf, audio.mp3, audio.ogg, Architecture Diagram.png\n"""; EMBED=re.compile(r"!\[\[([^\]]+)\]\]")
def validate(root:Path,config):
    md=iter_files(root,(".md",),parse_items(config.get("ignore","patterns",fallback=""))); base=root.parent if root.is_file() else root; all_files=[p for p in base.rglob("*") if p.is_file()]; by_name={p.name.lower():p for p in all_files}; ignored_targets=parse_items(config.get("rules","ignore_targets",fallback="")); referenced=set(); issues=[]
    for path in md:
        ...
        for m in EMBED.finditer(text):
            ...
            if any(fnmatch.fnmatchcase(target.lower(),pattern.lower()) for pattern in ignored_targets): continue
            referenced.add(Path(target).name.lower()); line=text.count("\n",0,m.start())+1
            if Path(target).name.lower() not in by_name: issues.append(Issue("error","ATT001",f"Embedded target {target!r} does not exist.",rel,line))
    folder=base/config.get("rules","attachments_directory")
    if folder.exists():
        for path in folder.rglob("*"):
            if path.is_file() and path.name.lower() not in referenced: issues.append(Issue("warning","ATT002","Attachment is not referenced by a Markdown file.",relative(path,root)))
    return issues,len(md)
