...
VALIDATOR_NAME="sections"; DEFAULT_INI="""[ignore]\npatterns =\n[rules]\npermanent = Purpose, Overview\ncontextual = Related\n"""
def validate(root:Path,config):
    files=iter_files(root,(".md",),parse_items(config.get("ignore","patterns",fallback=""))); issues=[]
    for path in files:
        try: fm,_=load_frontmatter(path)
        except Exception: continue
        if not fm or fm.get("kind")!="note": continue
        text=path.read_text(encoding="utf-8"); found=headings(text); typ=fm.get("type"); rel=relative(path,root)
        required=parse_items(config.get("rules",str(typ),fallback=""))
        if required and not any(item in found for item in required): issues.append(Issue("warning","SEC001",f"Consider adding one of these sections: {', '.join(required)}.",rel))
