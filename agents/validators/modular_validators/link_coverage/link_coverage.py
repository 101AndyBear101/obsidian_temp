...
VALIDATOR_NAME="link_coverage"; DEFAULT_INI="""[ignore]\npatterns = templates/**, plans/**, bases/**, agents/**, journals/daily/**\n[rules]\ncheck_inbound = true\ncheck_outbound = true\n"""; LINK=re.compile(r"(?<!!)\[\[([^\]]+)\]\]")
def validate(root:Path,config):
    files=iter_files(root,(".md",),parse_items(config.get("ignore","patterns",fallback=""))); names={p.stem.lower():p for p in files}; inbound=set(); outbound=set(); issues=[]
    for path in files:
        for m in LINK.finditer(path.read_text(encoding="utf-8")):
            outbound.add(path); inbound.add(Path(m.group(1).split("|",1)[0].split("#",1)[0]).name.lower())
    for name,path in names.items():
        rel=relative(path,root)
        if config.getboolean("rules","check_inbound") and name not in inbound: issues.append(Issue("warning","COV001","Consider adding an inbound wikilink to improve discovery.",rel))
        if config.getboolean("rules","check_outbound") and path not in outbound: issues.append(Issue("warning","COV002","Consider linking this note to related content.",rel))
