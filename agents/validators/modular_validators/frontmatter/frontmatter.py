...
VALIDATOR_NAME = "frontmatter"
DEFAULT_INI = """[ignore]\npatterns = templates/**\n[values]\nkinds = note, project, resource, area, plan, index, guide, template\nstatuses = active, paused, complete, archive\nnote_types = permanent, fleeting, contextual, diagram, journal\n"""


def validate(root: Path, config) -> tuple[list[Issue], int]:
    patterns = parse_items(config.get("ignore", "patterns", fallback=""))
    kinds = set(parse_items(config.get("values", "kinds")))
    statuses = set(parse_items(config.get("values", "statuses")))
    types = set(parse_items(config.get("values", "note_types")))
    files = iter_files(root, (".md",), patterns)
    issues: list[Issue] = []
    for path in files:
        ...
        try:
            ...
            issues.append(Issue("error", "META001", str(exc), rel, 1))
            continue
        if fm is None:
            issues.append(Issue("error", "META002", "Markdown file has no YAML frontmatter.", rel, 1))
            continue
        if path.name == "SKILL.md" and "name" in fm and "kind" not in fm:
            missing = [key for key in ("name", "description", "status", "tags") if not fm.get(key)]
        else:
            missing = [key for key in ("kind", "status", "created") if not fm.get(key)]
            if fm.get("kind") not in kinds:
                issues.append(Issue("error", "META003", f"Unsupported kind: {fm.get('kind')!r}.", rel, 1))
            if fm.get("status") not in statuses:
                issues.append(Issue("error", "META004", f"Unsupported status: {fm.get('status')!r}.", rel, 1))
            created = fm.get("created")
            if created and not isinstance(created, date):
                if not isinstance(created, str) or not re.fullmatch(r"\d{4}-\d{2}-\d{2}", created):
                    issues.append(Issue("error", "META005", "Created must be a real YYYY-MM-DD date.", rel, 1))
                else:
                    try: date.fromisoformat(created)
                    except ValueError: issues.append(Issue("error", "META005", "Created must be a real YYYY-MM-DD date.", rel, 1))
            if fm.get("kind") == "note":
                if not fm.get("type"):
                    issues.append(Issue("warning", "META006", "Consider adding a note type to improve classification.", rel, 1))
                elif fm.get("type") not in types:
                    issues.append(Issue("error", "META007", f"Unsupported note type: {fm.get('type')!r}.", rel, 1))
        for key in missing:
            issues.append(Issue("error", "META008", f"Required field {key!r} is missing or empty.", rel, 1))
        if not fm.get("tags"):
            issues.append(Issue("warning", "META009", "Consider adding tags to improve discovery.", rel, 1))
        elif not isinstance(fm.get("tags"), list):
            issues.append(Issue("error", "META010", "Tags must be a YAML list.", rel, 1))
