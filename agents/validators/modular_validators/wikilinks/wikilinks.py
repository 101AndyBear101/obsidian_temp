...
VALIDATOR_NAME="wikilinks"; DEFAULT_INI="""[ignore]\npatterns =\n[rules]\nignore_targets = note, embed, wikilinks, note-name, notes/note-name, notes/related-note, project-name, resource-name, area-name, skill-name, Note, Note Name, MyBase, BaseFile, improve workflow, Algorithm Notes, Meeting Notes 2024-01-10, ..., [Your other note, Other Note\nignore_empty_targets = true\n"""; LINK=re.compile(r"(?<!!)\[\[([^\]]+)\]\]")
def validate(root:Path,config):
    files=iter_files(root,(".md",),parse_items(config.get("ignore","patterns",fallback=""))); index={p.stem.lower():p for p in files}; ignored_targets=parse_items(config.get("rules","ignore_targets",fallback="")); ignore_empty=config.getboolean("rules","ignore_empty_targets",fallback=False); issues=[]
    for path in files:
        text=path.read_text(encoding="utf-8"); rel=relative(path,root)
        for match in LINK.finditer(text):
            target=match.group(1).split("|",1)[0]; name,_,heading=target.partition("#")
            if (not name and ignore_empty) or any(fnmatch.fnmatchcase(name.lower(),pattern.lower()) for pattern in ignored_targets): continue
            candidate=index.get(Path(name).name.lower())
            line=text.count("\n",0,match.start())+1
            if not candidate: issues.append(Issue("error","LINK001",f"Wikilink target {name!r} does not exist.",rel,line)); continue
            if heading and not re.search(rf"^#+\s+{re.escape(heading)}\s*$",candidate.read_text(encoding="utf-8"),re.M): issues.append(Issue("error","LINK002",f"Heading {heading!r} does not exist in {name!r}.",rel,line))
