...
VALIDATOR_NAME="bases"; DEFAULT_INI="""[ignore]\npatterns =\n[rules]\nallowed_kinds = note, project, resource, area, plan, index, guide, template\n"""
FOLDER=re.compile(r'file\.inFolder\(\s*"([^"]+)"\s*\)'); KIND=re.compile(r'kind\s*[=!]=\s*"([^"]+)"')
def validate(root:Path,config):
    base=root.parent if root.is_file() else root; files=iter_files(root,(".base",),parse_items(config.get("ignore","patterns",fallback=""))); kinds=set(parse_items(config.get("rules","allowed_kinds"))); issues=[]
    for path in files:
        text=path.read_text(encoding="utf-8"); rel=relative(path,root)
        for folder in FOLDER.findall(text):
            if not (base/folder.lstrip("/")).exists(): issues.append(Issue("error","BASE001",f"Base references missing folder {folder!r}.",rel))
        for kind in KIND.findall(text):
            if kind not in kinds: issues.append(Issue("error","BASE002",f"Base references unsupported kind {kind!r}.",rel))
